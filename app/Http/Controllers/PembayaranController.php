...
*Pemberitahuan Pembayaran ' . $nama_pembayaran . '*
    ...
    Diberitahukan kepada orang tua/wali dari Ananda/Anandi Siswa dengan detail sebagai berikut : 
...
*-Uang Gedung - T.A 2022-2023* 
*Sebesar : Rp. 1.000.000,00*

*-Pendaftaran - T.A 2022-2023* 
*Sebesar : Rp. 1.000.000,00*

*Jumlah Seluruh : Rp. 5.600.000,00*
...
*_Abaikan jika sudah membayar_*
    ...
    public function buktii(Request $request)
    {
        // dd($request->tipe);
        if ($request->tipe == 'Bulanan') {
            $t_bulanan = TransaksiBulanan::with('Siswa')->where('id', $request->id)->get();
            ...
            // dd($transaksi);
            return view('siswa.layout.bukti_pembayaran', compact('t_bulanan'));
        } elseif ($request->tipe == 'Bebas') {
            $t_bebas = TransaksiBebas::with('Siswa')->where('id', $request->id)->get();
            // dd($t_bebas);
            foreach ($t_bebas as $transaksi) {
            ...
            return view('siswa.layout.bukti_pembayaran', compact('t_bebas'));
        }
